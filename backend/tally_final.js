...
for (let i = 8; i < filas.length; i++) {
  ...
  if (cat === 'PASANTÍA') pasantiasKeyword.push({ fila: i+1, codigo: f[1], nombre, esInternacionalPorPais: !!contienePaisExtranjero(nombre) });
  if (!cat) {
...
console.log();
console.log('=== Dentro de las', pasantiasKeyword.length, 'filas con PASANTÍA en el nombre ===');
const conPais = pasantiasKeyword.filter(p => p.esInternacionalPorPais);
const sinPais = pasantiasKeyword.filter(p => !p.esInternacionalPorPais);
console.log('mencionan pais extranjero explicito:', conPais.length);
console.log('NO mencionan pais (posible nacional):', sinPais.length);
console.log('--- ejemplos que dicen NACIONAL explicitamente ---');
pasantiasKeyword.filter(p => quitarTildes(p.nombre.toUpperCase()).includes('NACIONAL')).forEach(p => console.log(p.fila, p.codigo, '|', p.nombre));
console.log('--- muestra de "sin pais" (primeras 15) ---');
sinPais.slice(0,15).forEach(p => console.log(p.fila, p.codigo, '|', p.nombre));
