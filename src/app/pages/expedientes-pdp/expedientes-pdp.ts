...
export class ExpedientesPdp implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.rolUsuario === 'Sectorista') {
      this.redesSectorista = this.redFiltro.split(',').map(r => r.trim()).filter(Boolean);
  ...
  private cargarEjecutores() {
    ...
    this.ejecutoresDisponibles = [...merged, ...nuevos]
      .filter(u => u.rol === 'Ejecutor' && u.estado !== 'Inactivo');
  }
  ...
  guardarAsignacion() {
    ...
    const nuevaAsig = { red: this.asignRedSeleccionada, ejecutorDni: this.asignEjecutorDni, ejecutorNombre: ejecutor.nombre };
    ...
    if (eidx !== -1) {
      ...
      usuarios.push({ dni: this.asignEjecutorDni, nombre: ejecutor.nombre, rol: 'Ejecutor', estado: 'Activo', sedes: this.asignRedSeleccionada });
    }
