...
export class ListaParticipantes implements OnInit {
  ...
  paginaAnterior() {
    if (this.pagina > 1) { this.pagina--; this.cargarParticipantes(); }
  }

  paginaSiguiente() {
    if (this.pagina < this.totalPaginas) { this.pagina++; this.cargarParticipantes(); }
  }
  ...
  buscarPorDni() {
    if (this.nuevo.dni_ce && this.nuevo.dni_ce.length >= 8) {
      this.pdpData.getPersonalPorDni(this.nuevo.dni_ce).subscribe({
        ...
        error: () => { /* DNI no encontrado, el usuario lo llena manualmente */ }
      });
    }
  }
  ...
  private vacio(): Participante {
    return {
      codigo_act: '', dni_ce: '', cod_planilla: '',
      nombre: '', apellidos: '', sexo: '',
      red: '', sub_programa: '', servicio_area: '',
      cargo: '', regimen_laboral: '',
    };
  }
}
